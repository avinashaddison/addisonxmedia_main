...
- Build Command: `bash scripts/render-build.sh` (corepack→pnpm; npm/yarn are rejected by the root `preinstall` guard — that was the original deploy failure)
- Start Command: `cd artifacts/api-server && node_modules/.bin/tsx server/index.ts` (package-local tsx, no pnpm needed at runtime)
